...
# Optional manual search using saved settings; no need to wait for the daily interval:
python backend/manage.py configure_autonomy --enable --run-now
```

The research worker schedules a cycle immediately, then checks for new work every minute. Default retraining is every 24 hours with at least 1,440 new completed spot candles per asset since the last successful snapshot; insufficient data postpones the check by five minutes. Intervals are bounded to 6–168 hours, history to 5,000–100,000 candles per asset, and leverage to an integer 1–10. The cap is validated again in position sizing and fill accounting. The **Run search now** control queues one manual cycle using saved settings, bypassing only the daily interval and fresh-candle minimum; active/queued work is deduplicated and snapshot freshness still applies. When that manual search finishes, the previous paper cycle flattens on fresh books before the replacement starts. This can reuse historical evidence, so it is not a new unseen test. Retrying failures waits one hour. An interrupted worker requeues the same immutable cutoff, rather than silently incorporating newer data. Disabling or changing settings cancels queued/ready cycles and closes active portfolios on fresh futures books; an in-progress fit can finish but cannot activate under changed settings.

Each cycle intersects the four assets' available date ranges and uses a common chronological 40% price-model fitting / 20% strategy development / 20% final selection / 20% historical holdout split. Within the initial 40%, the first three quarters fit candidate logistic classifiers and the final quarter selects C ∈ {0.01, 0.1, 1}; crossing outcome labels are purged. The chosen classifier and scaler are then refitted using only that initial 40%. All four directional/horizon models (long/short, 15/60 minutes) are frozen before strategy development begins. No scaler, classifier, search regressor or search proposal can use the later selection or holdout outcomes.

The strategy optimizer is our own per-asset scikit-learn pipeline. It samples a reproducible pool of 2,048 distinct numeric rule combinations and actually backtests 96 of them in four rounds. The first round explores 24 random combinations. Later rounds fit an `ExtraTreesRegressor` to the completed development results, choose 16 combinations by predicted objective plus half the disagreement between trees, and retain 8 random exploration choices. Tree disagreement is only a search heuristic, not a calibrated confidence interval. Each candidate is evaluated on two chronological halves of the development period with separate initial balances. The objective is the worse fold's net return minus 0.25 times the larger drawdown, minus 2 percentage points per missing minimum trade, and minus 100 for any liquidation, risk halt or drawdown-limit breach. Only candidates meeting development risk/trade gates can become finalists.

Search dimensions include long/short direction; trend, RSI, breakout or model-probability entries; optional trend, momentum and volatility filters joined with AND; SMA periods, RSI thresholds, breakout lookback and probability thresholds; reversal, RSI, momentum, confidence or timed exits; 15–240 minute holding limits; stop and profit target distances; and integer leverage up to the configured cap. Stops and targets use 25%, 50%, 75% or 100% of their configured maximum, with a 0.1% minimum. The search cannot change capital, allocation, planned equity risk, cost assumptions, drawdown or daily loss limits. The final rules are plain versioned JSON, interpreted by the same standard-library predicate in history and live paper trading. No generated code, pickle loading, hosted model or API key is involved.

The best 12 eligible development combinations per asset are frozen before the later 20% selection period. They compete against the original 12 fixed strategy families at every integer leverage up to the cap. At 10× the cycle has 384 development combinations plus 480 fixed-family combinations (864 distinct combinations actually backtested), and at most 528 candidates evaluated on final selection. The 2,048-item per-asset pool is not advertised as 2,048 completed backtests. Final selection ranks net return after fees, assumed spread, adverse slippage and adverse funding accrual. Eligibility requires at least five closed trades, zero modeled liquidations, drawdown below the configured ceiling and no final halt. Numerical ties (eight decimal places) prefer lower drawdown, then lower leverage.

Before the final historical holdout, each asset freezes its selected strategy, best searched combination and best fixed-family baseline. Their holdout metrics are reported side by side. Holdout outcomes never feed the optimizer or select a replacement. The designated capital stance stays cash unless the single overall validation winner has positive validation and holdout returns and passes the same gates. A baseline is allowed to win. This bounded search identifies the best eligible candidate among those evaluated, not every possible combination or a guaranteed future winner. Repeated historical windows overlap and are not repeatedly presented as fresh unseen evidence.

For each asset, its highest-ranked eligible candidate starts an independent virtual challenger after the cycle freezes, even if its validation result is negative. A fixed baseline shadow also starts with the same virtual capital and live feed. Selected and baseline wallets, decisions, fills and completed results stay separate. These balances are experimental comparisons, not a combined real allocation. Actual future observations produce the forward evidence. Positions fill only on a later recorded Bybit futures book with depth participation and adverse slippage; entry proposals are rechecked against fresh candles, futures books/marks, executed spot flow, funding, liquidations and optional news/heatmaps. Those live context vetoes cannot be retrospectively reconstructed from candles, so historical scores evaluate the candle rules and forward results evaluate the complete execution policy. Reasons, available inputs and fills are stored durably. Paper state and event offsets commit atomically and survive restarts. Existing spot paper runs and live-readiness gates are independent.

Paper notional is bounded by both cash committed to isolated margin plus entry fees and planned equity risk at the price stop including costs. More leverage does not automatically mean more exposure when the risk budget already binds. Defaults are 10,000 USDT per virtual portfolio, at most 10% cash committed, 0.5% planned equity risk per trade, a maximum 2% price stop and 4% profit target for combination search (fixed baselines use those full distances), 10% maximum drawdown and 3% daily loss limit. These are simulation controls, not guarantees against gaps. A position can remain open across a feed outage until fresh execution data arrives. An expired cycle closes positions before the next ready cycle activates; no model changes an existing position.
...
The full cycle JSON endpoint `/api/autonomy/cycles/<id>/` exposes every development trial, proposal source, surrogate estimate, fold metrics, final candidate metrics, model-fit evidence, data hashes, settings and limitations to authenticated users. The dashboard keeps the smaller round summaries, parameter importance and per-asset comparisons; its one-second updates do not transfer the full search ledger. Dashboard leadership and historical reports refresh every 30 seconds; current forward paper status and reasons refresh every second. Four trained models are stored per asset per cycle; all artifacts, policies, decisions and results persist in PostgreSQL. Monitor database growth as recordings and decision journals accumulate.
