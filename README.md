...
**Automatic AI research:** the **Automatic AI** tab runs our own scikit-learn strategy search separately for BTC, XRP, SOL and ETH. An ExtraTrees model learns from completed development backtests and iterates over 96 combinations per asset: entry signals and filters, exit conditions, stops, targets, holding periods and leverage up to the hard **10×** ceiling. Later validation compares the frozen finalists with fixed-family baselines; a separate holdout evaluates the selected choices without feeding the search. The dashboard shows exact rules, search rounds and per-asset comparisons. Selected strategies and baseline shadows then run on future Bybit perpetual depth and marks with equal virtual capital. Inference uses frozen numeric rules and JSON model weights in the trading worker; no hosted AI or API key is needed. Historical spot prices remain a futures proxy and funding an estimated adverse allowance. The system may choose cash, makes no future-profit guarantee and remains paper-only. See the operating guide for assumptions and controls.
...
Trained market models are available in **Trading operations → Test strategies**. Each asset gets a regularized classifier trained on up to 100,000 recorded candles, with chronological validation and cost-aware final testing. New unlinked paper runs freeze the latest model in a fourth portfolio. The price model needs no API key; optional AI news remains separate. Operators can queue all four with `python backend/manage.py train_models --enqueue --count 100000`. See [model methodology](docs/trading-system.md#trained-market-models).
